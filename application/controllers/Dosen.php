...
class Dosen extends CI_Controller {
	...
	function index($id=NULL){
		$jml = $this->db->get_where('family', array('kategori'=>'Dosen'));
		...
		$config['last_tagl_close'] = "</li>";
         ...
         //tamplikan data
         $data['query'] = $this->M_family->ambil_family($config['per_page'], $id);
		$data=array('title'=>'Dosen',
					'isi'=>'vDosen',
					'halaman'=>$this->pagination->create_links(),
					'jumlah'=>$jml->num_rows(),
					'hasil'=>$this->M_family->ambil_family($config['per_page'], $id)
					);
